...
public class ImportBPartner extends SvrProcess implements ImportProcess {
    ...
    protected void prepare() {
        ...
        for (int i = 0; i < para.length; i++) {
            ...
            if (name.equals("AD_Client_ID")) m_AD_Client_ID = ((BigDecimal) para[i].getParameter()).intValue(); else if (name.equals("DeleteOldImported")) m_deleteOldImported = "Y".equals(para[i].getParameter()); else if (name.equals("IsValidateOnly")) p_IsValidateOnly = para[i].getParameterAsBoolean(); else log.log(Level.SEVERE, "Unknown Parameter: " + name);
        }
