...
public class PiecewiseLinearFunction implements PiecewiseFunction {
    ...
    private double calculateY(double x1, double y1, double slope, double x) {
        if (slope != 0) return y1 + slope * (x - x1); else return y1;
    }
    ...
    public void setPoints(double x[], double y[]) {
        if (x.length != y.length) throw new IllegalArgumentException("Number of points in array X must be equal to number of values in array Y");
