...
class Solver {
    ...
    void reconstruct_gradient() {
        ...
        int i;
        for (i = active_size; i < l; i++) G[i] = G_bar[i] + p[i];
    ...
    void do_shrinking() {
        ...
        for (i = 0; i < active_size; i++) if (be_shrunken(i, Gmax1, Gmax2)) {
            active_size--;
            while (active_size > i) {
        ...
        unshrinked = true;
        reconstruct_gradient();
        for (i = l - 1; i >= active_size; i--) if (!be_shrunken(i, Gmax1, Gmax2)) {
            while (active_size < i) {
                if (be_shrunken(active_size, Gmax1, Gmax2)) {
                    swap_index(i, active_size);
                    break;
...
final class Solver_NU extends Solver {
    ...
    void do_shrinking() {
        ...
        for (i = 0; i < active_size; i++) if (be_shrunken(i, Gmax1, Gmax2, Gmax3, Gmax4)) {
            active_size--;
            while (active_size > i) {
        ...
        unshrinked = true;
        reconstruct_gradient();
        for (i = l - 1; i >= active_size; i--) if (!be_shrunken(i, Gmax1, Gmax2, Gmax3, Gmax4)) {
            while (active_size < i) {
                if (be_shrunken(active_size, Gmax1, Gmax2, Gmax3, Gmax4)) {
                    swap_index(i, active_size);
                    break;
    ...
    public static svm_model svm_train(svm_problem prob, svm_parameter param) {
        ...
        if (param.svm_type == svm_parameter.ONE_CLASS || param.svm_type == svm_parameter.EPSILON_SVR || param.svm_type == svm_parameter.NU_SVR) {
            ...
            for (i = 0; i < param.nr_weight; i++) {
                ...
                if (j != nr_class) weighted_C[j] *= param.weight[i];
            }
