...
public class WebdavSiteScape implements BasicWebdavStore, WebdavStoreBulkPropertyExtension, WebdavStoreLockExtension, WebdavStoreMacroCopyExtension, WebdavStoreMacroMoveExtension, WebdavStoreMacroDeleteExtension {
    ...
    public String[] getChildrenNames(String folderUri) throws ServiceAccessException, AccessDeniedException, ObjectNotFoundException, ObjectLockedException {
        try {
            ...
            } else if (uptoUriTypeOnly(m)) {
                if (zoneName != null) return new String[] { zoneName }; else return new String[0];
    ...
    public void lockObject(String uri, String lockId, String subject, Date expiration, boolean exclusive, boolean inheritable, String owner) throws ServiceAccessException, AccessDeniedException {
        ...
        if (inheritable) throw new AccessDeniedException(uri, "Recursive locking is not supported", "lock");
        String[] id = Util.parseSubject(subject);
        if (!id[0].equals(this.zoneName) || !id[1].equals(this.userName)) throw new AccessDeniedException(uri, "Cannot obtain lock on behalf of another user", "lock");
        try {
            Map m = parseUri(uri);
    ...
    private Map parseUri(String uri) throws ZoneMismatchException {
        ...
        String zname = u[2];
        if (!zname.equals(this.zoneName)) throw new ZoneMismatchException("No access to the specified zone");
        ...
        if (type.equals(URI_TYPE_INTERNAL)) {
            ...
            if (itemType.equals(URI_ITEM_TYPE_LIBRARY)) {
                ...
                if (u.length == 7) return returnMap(map, URI_SYNTACTIC_TYPE_FOLDER);
