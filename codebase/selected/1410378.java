 ...
 * @author Guy Arieli
 */
public class HtmlTestReporter implements ExtendTestReporter, ExtendTestListener, Externalizable {

    protected static Logger log = Logger.getLogger(HtmlTestReporter.class.getName());
    ...
    public HtmlTestReporter() throws Exception {
        if (!JSystemProperties.getInstance().isReporterVm()) {
            throw new Exception("You are trying to activate the HTML reporter from the test, while test is communicating with the runner. " + "A common reason for this problem is activating the reporter from the RunnerListenersManager. Use the ListenersManager instead");
        }
        boolean loadReportersState = RunnerStatePersistencyManager.getInstance().getLoadReporters();
        if (loadReportersState) {
            return;
        }
        updateLogDir();
        isZipLogDisable = ("true".equals(JSystemProperties.getInstance().getPreference(FrameworkOptions.HTML_ZIP_DISABLE)));
    ...
    public void saveFile(String fileName, byte[] content) {
        try {
            File file = new File(ListenerstManager.getInstance().getCurrentTestFolder(), fileName);
    ...
    public void report(String title, String message, int status, boolean bold, boolean html, boolean link) {
        ...
        if (link) {
            ...
            if (status == Reporter.FAIL) {
                lastTestStatus = Reporter.FAIL;
            } else if (lastTestStatus == Reporter.PASS && status == Reporter.WARNING) {
                lastTestStatus = Reporter.WARNING;
            }
