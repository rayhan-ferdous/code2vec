 ...
 * @since Alpha
 */
public final class AdminMainPanel extends JPanel implements ActionListener {
    ...
    public AdminMainPanel(JAdmin admin, Frame mainFrame) throws Exception {
        ...
        masterAlarm = new Indicator(new ImageIcon(this.getClass().getResource("/images/masteralarm_on.gif")), new ImageIcon(this.getClass().getResource("/images/masteralarm_off.gif")), masterAlarmSound);
    ...
    synchronized void showDisconnected(boolean isInitializing) {
        if (isInitializing) writeToEventLog("Initializing " + JAdmin.longVersionString); else writeToEventLog("Disconnected!");
                        ...
                        if (AdminMainPanel.this.currentState == STATE_CONNECTED) {
                            if ((customPanels != null) && (customPanels.size() > 0)) {
                                for (int i = 0; i < customPanels.size(); i++) {
                                    ...
                                    if (title != null) tabControl.addTab(title, null, customPanel, title); else tabControl.addTab("Custom panel " + i, null, customPanel, "Custom panel " + i);
                                }
                            }
                        }
                    }
    ...
    public void actionPerformed(final ActionEvent event) {
        if (event.getSource() == connect) {
            ...
            if (soundsEnabled) soundButton.setToolTipText("Click to disable sounds"); else soundButton.setToolTipText("Click to enable sounds");
        }
    }
    ...
    synchronized void doConnect() {
        if ((connectDialog == null) && (this.currentState != STATE_CONNECTED)) {
            try {
	 ...
	 */
    public final synchronized void writeToEventLog(final String event) {
