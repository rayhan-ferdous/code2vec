...
public class NewArrayNodeSet extends AbstractNodeSet implements ExtNodeSet, DocumentSet {
                ...
                if (add) {
                    switch(mode) {
                        case NodeSet.DESCENDANT:
                            ...
                            if (copyMatches) nodes[i].addMatches(parent);
                            result.add(nodes[i]);
                            break;
                        case NodeSet.ANCESTOR:
                            ...
                            if (copyMatches) parent.addMatches(nodes[i]);
                            result.add(parent, 1);
                            break;
                    }
                }
    ...
    int removeDuplicates(boolean mergeContext) {
        ...
        for (int i = 1; i < size; i++) {
            if (nodes[i].compareTo(nodes[j]) != 0) {
                ...
                if (mergeContext) nodes[j].addContext(nodes[i]);
    ...
    public NodeSet selectFollowing(NodeSet pl, int position, int contextId) throws XPathException, UnsupportedOperationException {
        ...
        for (NodeProxy reference : pl) {
            ...
            for (; i < size; i++) {
                if (nodes[i].getDocument().getDocId() != reference.getDocument().getDocId() || (nodes[i].compareTo(reference) > 0 && !nodes[i].getNodeId().isDescendantOf(reference.getNodeId()))) break;
            }
            int n = 0;
            for (int j = i; j < size; j++) {
                if (nodes[j].getDocument().getDocId() != reference.getDocument().getDocId()) break;
    ...
    public NodeSet selectPreceding(NodeSet pl, int position, int contextId) throws XPathException, UnsupportedOperationException {
        ...
        for (NodeProxy reference : pl) {
            ...
            int i = documentOffsets[idx];
            for (; i < size; i++) {
                if (nodes[i].compareTo(reference) >= 0) break;
            }
            --i;
            int n = 0;
    ...
    public NodeSet except(NodeSet other) {
        ...
        for (int i = 0; i < size; i++) {
            if (!other.contains(nodes[i])) result.add(nodes[i]);
        }
        return result;
    }
    ...
    public Iterator<DocumentImpl> getDocumentIterator() {
        sort();
        if (docIter != null) return docIter.reset();
    ...
    private class DocumentIterator implements Iterator<DocumentImpl> {
        ...
        public DocumentImpl next() {
            if (currentDoc == documentCount) return null; else return nodes[documentOffsets[currentDoc++]].getDocument();
        }
    ...
    public boolean equalDocs(DocumentSet other) {
        ...
        sort();
        if (documentCount != other.getDocumentCount()) return false;
        for (int i = 0; i < documentCount; i++) {
            if (!other.contains(documentIds[i])) return false;
        }
        return true;
    }
    ...
    public DocumentImpl getDocumentAt(int pos) {
        sort();
        if (pos < 0 || pos >= documentCount) return null;
    ...
    public boolean contains(DocumentSet other) {
        sort();
        if (other.getDocumentCount() > documentCount) return false;
        DocumentImpl d;
        for (Iterator<DocumentImpl> i = other.getDocumentIterator(); i.hasNext(); ) {
            d = i.next();
    ...
    public Iterator<Collection> getCollectionIterator() {
        ...
        if (cachedCollections == null) {
            ...
            for (int i = 0; i < documentCount; i++) {
                ...
                if (!cachedCollections.contains(doc.getCollection())) cachedCollections.add(doc.getCollection());
            }
        }
