...
public class ItemTable {
    ...
    public static ItemTable getInstance() {
        if (_instance == null) _instance = new ItemTable();
        return _instance;
    }
    ...
    public ItemTable() {
        ...
        try {
            ...
            for (String selectQuery : SQL_ITEM_SELECTS) {
                if (selectQuery.endsWith("_etcitem") && !Config.CUSTOM_ETCITEM_TABLE) continue; else if (selectQuery.endsWith("_armor") && !Config.CUSTOM_ARMOR_TABLE) continue; else if (selectQuery.endsWith("_weapon") && !Config.CUSTOM_WEAPON_TABLE) continue;
                PreparedStatement statement = con.prepareStatement(selectQuery);
                ResultSet rset = statement.executeQuery();
                while (rset.next()) if (selectQuery.endsWith("etcitem")) {
        ...
        for (L2Armor armor : SkillsEngine.getInstance().loadArmors(armorData)) _armors.put(armor.getItemId(), armor);
        _log.config("ItemTable: Loaded " + _armors.size() + " Armors.");
        for (L2EtcItem item : SkillsEngine.getInstance().loadItems(itemData)) _etcItems.put(item.getItemId(), item);
        _log.config("ItemTable: Loaded " + _etcItems.size() + " Items.");
        for (L2Weapon weapon : SkillsEngine.getInstance().loadWeapons(weaponData)) _weapons.put(weapon.getItemId(), weapon);
    ...
    private Item readItem(ResultSet rset) throws SQLException {
        ...
        } else if (consume.equals("stackable")) item.set.set("stackable", true); else item.set.set("stackable", false);
    ...
    private void buildFastLookupTable() {
        ...
        for (Iterator<Integer> iter = _armors.keySet().iterator(); iter.hasNext(); ) {
            ...
            if (item.getItemId() > highestId) highestId = item.getItemId();
        }
    ...
    public L2ItemInstance createDummyItem(int itemId) {
        ...
        if (temp.getItem() == null) _log.warning("ItemTable: Item Template missing for Id: " + itemId);
        return temp;
    }
