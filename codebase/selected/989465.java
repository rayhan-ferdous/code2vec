...
public final class SKEYDictionary {
    ...
    public static String bitsToEnglish(byte[] bits, int offset) {
        ...
        for (p = 0, i = 0; i < 64; i += 2) p += extractBits(bits2, i, 2);
        bits2[8] = (byte) (p << 6);
    ...
    public static byte[] englishToBits(String english) {
        ...
        for (i = 0, p = 0; i < 6; i++, p += 11) {
            if ((word = st.nextToken()) == null) return null;
            l = word.length();
            if (l > 4 || l < 1) {
                return null;
            } else if (l < 4) {
            ...
            if ((v = wsrch(word, low, high)) < 0) return null;
            insertBits(bits, v, p, 11);
        }
        for (p = 0, i = 0; i < 64; i += 2) p += extractBits(bits, i, 2);
        if ((p & 3) != extractBits(bits, 64, 2)) return null;
        byte[] out = new byte[8];
