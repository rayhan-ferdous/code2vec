...
public class AttributeSearchLookup extends AttributeNumberLookup {
    ...
    public void initComponent() {
        ...
        oSearchCommand.addActionListener(new SearchActionListener());
    ...
    public String getDisplayValue() {
        ...
        if (displayValue != null && displayValue.length() > 0) {
            String objectAttributeReferenceName = ((XEOObjectAttributeConnector) getDataFieldConnector()).getAttributeHandler().getDefAttribute().getReferencedObjectName();
            return "<img src='resources/" + objectAttributeReferenceName + "/ico16.gif' width='16' height='16' align='bottom' />" + super.getDisplayValue();
        } else {
            return "";
        }
    }
    ...
    public static class XEOHTMLRenderer extends XUIRenderer {
        ...
        public void encodeBegin(XUIComponentBase oComp) throws IOException {
            ...
            if (!oAttr.isRenderedOnClient()) {
                ...
                String width = "51px;";
                if (!oAttr.getShowFavorites()) width = "34px;";
                w.writeAttribute(STYLE, "vertical-align:top;width:" + width + "" + (oAttr.isUsable() ? "" : "display:none;"), null);
                w.endElement(COL);
                ...
                w.writeAttribute(STYLE, "top:0px;border:1px solid #9AB; margin-left:0x; margin-right:0px; padding-left:0px; padding-right:0px;" + (oAttr.isVisible() ? "" : "display:none;"), null);
        ...
        public void encodeEnd(XUIComponentBase oComp) throws IOException {
            ...
            if (!oAttr.isRenderedOnClient()) {
                ...
                if (!oAttr.isUsable()) w.writeAttribute(CLASS, "xwc-unusable", null);
                w.writeAttribute(STYLE, "width:100%;height:19px;border-width:0px;margin: 0x 0px 0px 0px; padding:0px 0px 0px 2px;display:none;" + "background-color:transparent;font-family: tahoma,verdana,helvetica;font-size: 12px;", null);
                w.endElement(INPUT);
                w.endElement(TD);
                w.startElement(TD, oComp);
                w.writeAttribute(ID, oAttr.getClientId() + "_tdBtns", null);
                w.writeAttribute(STYLE, "border-right:1px solid #9AB;border-bottom:1px solid #9AB;border-top:1px solid #9AB;" + (oAttr.isUsable() ? "" : "display:none;"), null);
                w.startElement(TABLE, oComp);
                w.writeAttribute(CELLPADDING, "0", null);
                w.writeAttribute(CELLSPACING, "0", null);
                w.startElement(TR, oComp);
                ...
                if (oAttr.isUsable()) w.writeAttribute(ONCLICK, XVWScripts.getAjaxCommandScript(oAttr.getLookupCommand(), XVWScripts.WAIT_DIALOG), null);
                w.writeAttribute(CLASS, "search-lookup-trigger", null);
                w.endElement(A);
                w.endElement(SPAN);
                w.endElement(DIV);
                ...
                w.writeAttribute(SRC, "javascript:void(0)", null);
                if (oAttr.isUsable()) w.writeAttribute(ONCLICK, getClearCode((Form) oAttr.findParentComponent(XUIForm.class), oAttr), null);
                ...
                if (oAttr.getShowFavorites()) {
                    ...
                    w.writeAttribute(SRC, "javascript:void(0)", null);
                    if (oAttr.isUsable()) w.writeAttribute(ONCLICK, b.toString(), null);
                ...
                getRequestContext().getScriptContext().add(XUIScriptContext.POSITION_FOOTER, "hideTbl" + oComp.getId(), scriptGenerator.generate());
            }
        }
        ...
        private class JavascriptGeneratorForVisibilityAndUsability {
            ...
            private String getDisplayValueForLookup() {
                if (oAttr.getEnableCardIdLink()) return oAttr.getDisplayValueCardIdLink(); else return oAttr.getDisplayValue();
            }
            ...
            private void setDisplayOfElement(String idComponent, boolean visible) {
                ...
                if (!visible) cssDisplayProperty = "none";
            ...
            private void setElementClass(StringBuilder builder, String idComponent, String cssClass, boolean add) {
                ...
                if (!add) extFunction = "removeClass(";
        ...
        private void addEventsToHtmlComponent(AttributeSearchLookup att, String clientId) {
            ...
            builder.append(XVWScripts.getAjaxCommandScript(att.getOpenCommand(), XVWScripts.WAIT_STATUS_MESSAGE));
            builder.append("};");
            ...
            builder.append("');");
            String width = "55px";
            if (!att.getShowFavorites()) width = "36px";
        ...
        private String getClearCode(Form oForm, AttributeBase oAttr) {
            if (oForm.haveDependents(oAttr.getObjectAttribute()) || oAttr.isOnChangeSubmit()) {
                ...
                return "document.getElementById('" + oAttr.getClientId() + "_ci').value='NaN';document.getElementById('" + oAttr.getClientId() + "_list').innerHTML=''";
            }
        }
