...
class Solver {
    ...
    void reconstruct_gradient() {
        ...
        int i;
        for (i = active_size; i < l; i++) G[i] = G_bar[i] + b[i];
        for (i = 0; i < active_size; i++) if (is_free(i)) {
            float[] Q_i = Q.get_Q(i, l);
            double alpha_i = alpha[i];
            for (int j = active_size; j < l; j++) G[j] += alpha_i * Q_i[j];
        }
    }

    void Solve(int l, QMatrix Q, double[] b_, byte[] y_, double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking) {
        this.l = l;
        this.Q = Q;
        QD = Q.get_QD();
        b = (double[]) b_.clone();
        ...
        si.rho = calculate_rho();
        {
            double v = 0;
            int i;
            for (i = 0; i < l; i++) v += alpha[i] * (G[i] + b[i]);
            si.obj = v / 2;
        }
        {
...
final class Solver_NU extends Solver {
    ...
    void Solve(int l, QMatrix Q, double[] b, byte[] y, double[] alpha, double Cp, double Cn, double eps, SolutionInfo si, int shrinking) {
        this.si = si;
...
class SVC_Q extends Kernel {
    ...
    float[] get_Q(int i, int len) {
        ...
        if ((start = cache.get_data(i, data, len)) < len) {
            for (int j = start; j < len; j++) data[0][j] = (float) (y[i] * y[j] * kernel_function(i, j));
        }
        return data[0];
    }
...
class ONE_CLASS_Q extends Kernel {
    ...
    float[] get_Q(int i, int len) {
        ...
        int start;
        if ((start = cache.get_data(i, data, len)) < len) {
            for (int j = start; j < len; j++) data[0][j] = (float) kernel_function(i, j);
        }
        return data[0];
    }
...
public class svm {
    ...
    public static void svm_predict_values(svm_model model, svm_node[] x, double[] dec_values, int start[]) {
        ...
        for (i = 0; i < l; i++) kvalue[i] = Kernel.k_function(x, model.SV[i], model.param, xlen);
        int p = 0;
        int pos = 0;
        int si, sj, ci, cj, k;
    ...
    public static String svm_check_parameter(svm_problem prob, svm_parameter param) {
        ...
        if (kernel_type != svm_parameter.LINEAR && kernel_type != svm_parameter.POLY && kernel_type != svm_parameter.RBF && kernel_type != svm_parameter.SIGMOID) return "unknown kernel type";
