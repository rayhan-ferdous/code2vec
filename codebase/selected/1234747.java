...
public class WebDriverTestingEngineImpl implements ITestingEngine {
    ...
    public void beginAt(URL aInitialURL, TestContext aTestContext) throws TestingEngineResponseException {
        ...
        for (javax.servlet.http.Cookie c : aTestContext.getCookies()) {
            ...
            driver.manage().addCookie(new org.openqa.selenium.Cookie(c.getName(), c.getValue(), domain, c.getPath() != null ? c.getPath() : "", expiry, c.getSecure()));
        }
    ...
    public void gotoPage(URL url) throws TestingEngineResponseException {
        ...
        driver.get(urlStr);
        throwFailingHttpStatusCodeExceptionIfNecessary(response.getStatusLine().getStatusCode(), urlStr);
    }
    ...
    private void throwFailingHttpStatusCodeExceptionIfNecessary(int statusCode, String url) {
        final boolean successful = (statusCode >= HttpStatus.SC_OK && statusCode < HttpStatus.SC_MULTIPLE_CHOICES) || statusCode == HttpStatus.SC_USE_PROXY || statusCode == HttpStatus.SC_NOT_MODIFIED;
        if (!this.ignoreFailingStatusCodes && !successful) {
            throw new TestingEngineResponseException(statusCode, "unexpected status code [" + statusCode + "] at URL: [" + url + "]");
        }
    }
    ...
    public boolean hasFrame(String frameNameOrId) {
        ...
        for (WebElement frame : frameset) {
            if (frameNameOrId.equals(frame.getAttribute("name")) || frameNameOrId.equals(frame.getAttribute("id"))) {
                return true;
            }
        }
        return false;
    ...
    public void setWorkingForm(String nameOrId, int index) {
        if (nameOrId != null) {
            formIdent = "(@name=" + escapeQuotes(nameOrId) + " or @id=" + escapeQuotes(nameOrId) + ")][position()=" + (index + 1);
        } else {
            formIdent = null;
        }
    }
    ...
    public void selectOptions(String selectName, int index, String[] optionValues) {
        Select select = new Select(getWebElementByXPath("//select[@name=" + escapeQuotes(selectName) + "][" + (index + 1) + "]", true, true));
        if (!select.isMultiple() && optionValues.length > 1) throw new RuntimeException("Multiselect not enabled");
    ...
    public boolean hasSubmitButton(String nameOrID) {
        return (getWebElementByXPath("//input[(@type='submit' or @type='image') and (@name=" + escapeQuotes(nameOrID) + " or @id=" + nameOrID + ")]", true, true) != null) || (getWebElementByXPath("//button[@type='submit' and (@name=" + escapeQuotes(nameOrID) + " or @id=" + escapeQuotes(nameOrID) + ")]", true, true) != null);
    }

    public boolean hasSubmitButton(String nameOrID, String value) {
        return (getWebElementByXPath("//input[(@type='submit' or @type='image') and (@name=" + escapeQuotes(nameOrID) + " or @id=" + escapeQuotes(nameOrID) + ") and @value=" + escapeQuotes(value) + "]", true, true) != null) || (getWebElementByXPath("//button[@type='submit' and (@name=" + escapeQuotes(nameOrID) + " or @id=" + escapeQuotes(nameOrID) + ") and @value=" + escapeQuotes(value) + "]", true, true) != null);
    }
    ...
    public void submit() {
        ...
        e.submit();
        throwFailingHttpStatusCodeExceptionIfNecessary(response.getStatusLine().getStatusCode(), driver.getCurrentUrl());
    }
    ...
    public Table getTable(String tableSummaryNameOrId) {
        ...
        for (int i = 0; i < trs.size(); i++) {
            ...
            for (WebElement td : tds) {
                ...
                newRow.appendCell(new Cell(td.getText(), colspan, rowspan));
            }
    ...
    private WebElement getLinkWithImage(String filename, int index) {
        try {
            return driver.findElement(By.xpath("(//a[img[contains(@src," + escapeQuotes(filename) + ")]])[" + (index + 1) + "]"));
        } catch (NoSuchElementException e) {
            return null;
        }
    }
