...
public class Chunkers {
    ...
    public static String toChunksStringEx(TextWithChunks text, int contextOffset, boolean withToString, String... showOnly) {
        StringBuilder sb = new StringBuilder();
        Collection<Chunk> set;
        if (showOnly.length == 0) {
            set = text;
        ...
        for (Chunk an : set) {
            String s = replaceEol(StringUtils.substring(text.getContent(), Math.max(an.start - contextOffset, 0), an.start) + "[[[" + an.getContent() + "]]]" + StringUtils.substring(text.getContent(), an.end, an.end + contextOffset));
            if (withToString) {
