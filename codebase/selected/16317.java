...
public class NativeModule extends Module {
    ...
    public String getFileName(boolean fullPath) {
        if (fullPath) return path.winPath + name;
        return name;
    }
    ...
    public void callDllMain(int dwReason) {
        if (header.imageOptional.AddressOfEntryPoint == 0) {
            if (WinAPI.LOG) System.out.println(name + " has no DllMain");
        } else {
            int esp = CPU_Regs.reg_esp.dword;
            CPU.CPU_Push32(0);
            CPU.CPU_Push32(dwReason);
            CPU.CPU_Push32(getHandle());
            if (returnEip == 0) {
            ...
            try {
                ...
                if (WinAPI.LOG) System.out.println(name + " calling DllMain SUCCESS");
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public boolean load(WinProcess process, int page_directory, String name, Path path) {
        ...
        try {
            ...
            if (!header.load(os, fis)) return false;
            ...
            for (int i = 0; i < header.imageSections.length; i++) {
                ...
                if (buffer.length > 0) fis.read(buffer);
                int size = buffer.length;
                if (header.imageSections[i].PhysicalAddress_or_VirtualSize > size) size = (int) header.imageSections[i].PhysicalAddress_or_VirtualSize;
                if (size == 0) size = (int) header.imageSections[i].SizeOfRawData;
                ...
                if (size > buffer.length) Memory.mem_zero(address + buffer.length, size - buffer.length);
            }
            if (oldbase != 0) {
    ...
    public int getAddressOfResource(int type, int id, IntRef size) {
        if (resourceStartAddress == 0) return 0;
        ...
        if (type > 0xFFFF) {
            ...
            for (int i = 0; i < root.NumberOfIdEntries; i++) {
                ...
                if (strId.equalsIgnoreCase(itemName)) {
                    if (offset < 0) return getResourceById(resourceStartAddress + (offset & 0x7FFFFFFF), id, size);
    ...
    private int getResource(int address, IntRef size) {
        int offset = Memory.mem_readd(address);
        if (size != null) size.value = Memory.mem_readd(address + 4);
        ...
        if (id > 0xFFFF) {
            ...
            for (int i = 0; i < root.NumberOfNamedEntries; i++) {
                ...
                if (itemName.equalsIgnoreCase(strId)) {
                    if (offset < 0) return getResourceByCodePage(resourceStartAddress + (offset & 0x7FFFFFFF), size);
                    return getResource(resourceStartAddress + offset, size);
                }
            }
        } else {
    ...
    private int getResourceByCodePage(int resourceAddress, IntRef size) {
        ...
        for (int i = 0; i < root.NumberOfIdEntries; i++) {
            ...
            int offset = Memory.mem_readd(address);
            address += 4;
            if (defaultOffset == 0) defaultOffset = offset;
            if (name == 1033) {
                return getResource(resourceStartAddress + offset, size);
            }
        }
        if (defaultOffset != 0) return getResource(resourceStartAddress + defaultOffset, size);
        return 0;
    }
    ...
    public boolean RtlImageDirectoryEntryToData(int dir, LongRef address, LongRef size) {
        if (dir >= header.imageOptional.NumberOfRvaAndSizes) return false;
        if (header.imageOptional.DataDirectory[dir].VirtualAddress == 0) return false;
    ...
    public Vector getImportDescriptors(long address) {
        ...
        try {
            ...
            while (true) {
                ...
                desc.load(file);
                if (desc.Name != 0 && desc.FirstThunk != 0) importDescriptors.add(desc); else break;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
