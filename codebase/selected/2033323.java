...
public class Encoder {
    ...
    public void pullSnapShots(File source, int number, int offSet, EncoderProgressListener listener) throws IllegalArgumentException, InputFormatException, EncoderException {
        FFMPEGExecutor ffmpeg = locator.createExecutor();
        ffmpeg.addArgument("-i");
        ffmpeg.addArgument(source.getAbsolutePath());
        ffmpeg.addArgument("-y");
        ...
        if (offSet > 0) {
            ffmpeg.addArgument("-ss");
            ffmpeg.addArgument(String.valueOf(offSet));
        }
        ffmpeg.addArgument("-vframes");
        ffmpeg.addArgument(number + "");
        ffmpeg.addArgument("-f");
        ffmpeg.addArgument("image2");
        ffmpeg.addArgument("-r");
        ffmpeg.addArgument("1");
        String targetFileName = source.getAbsolutePath();
        targetFileName = targetFileName.substring(0, targetFileName.lastIndexOf("."));
        ffmpeg.addArgument(targetFileName + "-%03d.jpg");
        try {
            ffmpeg.execute();
        } catch (IOException e) {
            throw new EncoderException(e);
