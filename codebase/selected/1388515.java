...
public class CollationPerformanceTest {
    ...
    void doKeyHist() {
        ...
        for (int i = 1; i <= maxLen; i++) {
            if (numKeysOfSize[i] > 0) {
                System.out.println(i + ", " + nf.format(accumulatedLen[i] / (numKeysOfSize[i] + 0.0)) + ", " + nf.format(accumulatedLen[i] / (numKeysOfSize[i] * i + 0.0)));
            }
        }
    }
    ...
    void doForwardIterTest() {
        ...
        System.out.println("Total number of strings compared " + tests.length + "in " + opt_loopCount + " loops");
