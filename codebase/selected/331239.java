...
public class WedgeEditor extends RigidShapeEditor {
    ...
    protected void assembleScaleControlPoints(DrawContext dc) {
        ...
        if (controlPoints.size() > 0) {
            ...
            Vec4 vert = matrix.transformBy3(matrix, 1, 0, 0).add3(refPt);
            ...
            vert = matrix.transformBy3(matrix, 0, 1, 0).add3(refPt);
            ...
            rod.setAttributes(this.scaleRodAttributes);
            this.controlPointRods.add(rod);
            vert = matrix.transformBy3(matrix, -1, 0, 0).add3(refPt);
            vertexPosition = this.wwd.getModel().getGlobe().computePositionFromPoint(vert);
            controlPoint = new Ellipsoid(vertexPosition, radius, radius, radius);
            controlPoint.setAttributes(this.scaleControlAttributes);
            controlPoint.setAltitudeMode(this.getAltitudeMode());
            ...
            vert = matrix.transformBy3(matrix, angleSin, angleCos, 1).add3(refPt);
            vertexPosition = this.wwd.getModel().getGlobe().computePositionFromPoint(vert);
            controlPoint = new Ellipsoid(vertexPosition, radius, radius, radius);
    ...
    protected void assembleTextureControlPoints(DrawContext dc) {
        ...
        if (selectedFace == 0) {
            ...
            ULeft = new Vec4(0 + x * shape.getOffsets(4, 0)[0], 0 + y * shape.getOffsets(4, 0)[0], 1 + shape.getOffsets(4, 0)[1]);
            URight = new Vec4(x + x * shape.getOffsets(4, 1)[0], y + y * shape.getOffsets(4, 1)[0], 1 + shape.getOffsets(4, 1)[1]);
            LLeft = new Vec4(0 + x * shape.getOffsets(4, 2)[0], 0 + y * shape.getOffsets(4, 2)[0], -1 + shape.getOffsets(4, 2)[1]);
            LRight = new Vec4(x + x * shape.getOffsets(4, 3)[0], y + y * shape.getOffsets(4, 3)[0], -1 + shape.getOffsets(4, 3)[1]);
        }
    ...
    protected void scaleShapeAngle(Point previousMousePoint, Point mousePoint) {
        ...
        if (vectorOnPlane.cross3(previousVectorOnPlane).dot3(perpendicularVector) > 0) rotationChange = Angle.fromRadians(rotationChange.getRadians() * -1);
