...
public class Tables {
    ...
    public List<InputTable> fromNodes(Nodes nodes) {
        ...
        for (Node node : nodeList) {
            try {
                ...
                rowValues.add(TableUtil.fromLatLng(node.getLatitude(), node.getLongitude()));
                values.add(rowValues);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public List<InputTable> fromGates(Gates gates) {
        ...
        gateTable.setHeaders(Arrays.asList(new String[] { "NAME", "FROM_OBJ", "FROM_IDENTIFIER", "TO_NODE" }));
        ArrayList<ArrayList<String>> values = new ArrayList<ArrayList<String>>();
        ...
        for (Gate gate : gates.getGates()) {
            try {
                ArrayList<String> rowValues = new ArrayList<String>();
                rowValues.add(gate.getName());
                rowValues.add(TableUtil.fromLatLng(gate.getLatitude(), gate.getLongitude()));
                values.add(rowValues);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public Gates toGates() {
        ...
        for (int i = 0; i < ngates; i++) {
            try {
                ...
                gate.setFromIdentifier(gateTable.getValue(i, "FROM_IDENTIFIER"));
        ...
        if (gateWeirTable != null) {
            ...
            for (int i = 0; i < ndevices; i++) {
                ...
                device.numberOfDuplicates = Integer.parseInt(gateWeirTable.getValue(i, "NDUPLICATE"));
                device.width = Double.parseDouble(gateWeirTable.getValue(i, "WIDTH"));
                device.elevation = Double.parseDouble(gateWeirTable.getValue(i, "ELEV"));
                device.height = Double.parseDouble(gateWeirTable.getValue(i, "HEIGHT"));
                device.coefficientFromNode = Double.parseDouble(gateWeirTable.getValue(i, "CF_FROM_NODE"));
                device.coefficientToNode = Double.parseDouble(gateWeirTable.getValue(i, "CF_TO_NODE"));
                device.defaultOperation = gateWeirTable.getValue(i, "DEFAULT_OP");
        ...
        if (gatePipeTable != null) {
            ...
            for (int i = 0; i < ndevices; i++) {
                ...
                device.numberOfDuplicates = Integer.parseInt(gatePipeTable.getValue(i, "NDUPLICATE"));
                device.radius = Double.parseDouble(gatePipeTable.getValue(i, "RADIUS"));
                device.elevation = Double.parseDouble(gatePipeTable.getValue(i, "ELEV"));
                device.coefficientFromNode = Double.parseDouble(gatePipeTable.getValue(i, "CF_FROM_NODE"));
                device.coefficientToNode = Double.parseDouble(gatePipeTable.getValue(i, "CF_TO_NODE"));
                device.defaultOperation = gatePipeTable.getValue(i, "DEFAULT_OP");
                Gate gate = gates.getGate(device.gateName);
                if (gate != null) {
                    gate.addGateDevice(device);
                }
    ...
    public List<InputTable> fromReservoirs(Reservoirs reservoirs) {
        ...
        reservoirTable.setHeaders(Arrays.asList(new String[] { "NAME", "AREA", "BOT_ELEV" }));
        ArrayList<ArrayList<String>> values = new ArrayList<ArrayList<String>>();
        ...
        reservoirConnectionTable.setHeaders(Arrays.asList(new String[] { "RES_NAME", "NODE", "COEF_IN", "COEF_OUT" }));
        values = new ArrayList<ArrayList<String>>();
        for (Reservoir reservoir : reservoirs.getReservoirs()) {
            for (ReservoirConnection reservoirConnection : reservoir.getReservoirConnections()) {
                try {
                    ArrayList<String> rowValues = new ArrayList<String>();
        ...
        gisTable.setHeaders(Arrays.asList(new String[] { "ID", "LAT_LNG", "INTERIOR_LAT_LNG" }));
        values = new ArrayList<ArrayList<String>>();
        for (Reservoir reservoir : reservoirs.getReservoirs()) {
            try {
                ArrayList<String> rowValues = new ArrayList<String>();
                rowValues.add(reservoir.getName());
                rowValues.add(TableUtil.fromLatLng(reservoir.getLatitude(), reservoir.getLongitude()));
                rowValues.add(TableUtil.fromLatLngPoints(reservoir.getLatLngPoints()));
                values.add(rowValues);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public Reservoirs toReservoirs() {
        ...
        for (int i = 0; i < nreservoirs; i++) {
            try {
                ...
                reservoir.setArea(Double.parseDouble(reservoirTable.getValue(i, "AREA")));
                reservoir.setBottomElevation(Double.parseDouble(reservoirTable.getValue(i, "BOT_ELEV")));
        ...
        for (int i = 0; i < nconnections; i++) {
            try {
                String reservoirName = reservoirConnectionTable.getValue(i, "RES_NAME");
                ReservoirConnection connection = new ReservoirConnection();
                connection.reservoirName = reservoirName;
                connection.nodeId = reservoirConnectionTable.getValue(i, "NODE");
                connection.coefficientIn = Double.parseDouble(reservoirConnectionTable.getValue(i, "COEF_IN"));
                connection.coefficientOut = Double.parseDouble(reservoirConnectionTable.getValue(i, "COEF_OUT"));
                Reservoir reservoir = reservoirs.getReservoir(reservoirName);
                if (reservoir == null) {
                    System.err.println("Connection for unknown reservoir named: " + reservoirName);
                }
            ...
            for (int i = 0; i < nvals; i++) {
                try {
                    ...
                    if (reservoir != null) {
                        ...
                        reservoir.setLatLngPoints(TableUtil.toLatLngPoints(gisTable.getValue(i, "INTERIOR_LAT_LNG")));
                    }
                } catch (Exception e) {
                }
            }
    ...
    public Transfers toTransfers() {
        ...
        if (transferTable != null) {
            ...
            for (int i = 0; i < ntransfers; i++) {
                ...
                transfer.fromIdentifier = transferTable.getValue(i, "FROM_IDENTIFIER");
                transfer.toObject = transferTable.getValue(i, "TO_OBJ");
                transfer.toIdentifier = transferTable.getValue(i, "TO_IDENTIFIER");
