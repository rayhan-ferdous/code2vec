...
public class TempFile {
    ...
    public static File getTmpDir() {
        ...
        if (!tempFile.exists()) if (!tempFile.mkdirs()) throw new RuntimeException("Can't initialize temporary directory: " + tempFile);
        return tempFile;
    }
    ...
    static File _mktmpdir(File tmpfile) throws IOException {
        tmpfile.delete();
        File parent = tmpfile.getParentFile();
        String name = tmpfile.getName();
        File tmpdir = new File(parent, name + ".d");
        int index = 0;
        while (tmpdir.exists()) tmpdir = new File(parent, name + index++ + ".d");
        if (!tmpdir.mkdir()) throw new IOException("Failed to mkdir " + tmpdir);
        tmpdir.deleteOnExit();
        return tmpdir;
    }

    public static File createTempFile() throws IOException {
        return createTempFile("freejava", null);
    }

    public static File createTempDirectory() throws IOException {
        return createTempDirectory("freejava", null);
    }
