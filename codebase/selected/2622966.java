...
/**
 * Version using Uncrypted file and FileChannel for In and for for Out
 * 
 * @author frederic
 * 
 */
public class ArkFsDocUnencryptedFC extends ArkFsDocAbstract {

    /**
     * Internal Logger
     */
    private static final GgInternalLogger logger = GgInternalLoggerFactory.getLogger(ArkFsDocUnencryptedFC.class);
     ...
     */
    public ArkFsDocUnencryptedFC(ArkStoreInterface storage, long did) throws ArUnvalidIndexException {
        super(storage, did);
    }
    ...
    public boolean isInReading() throws ArUnvalidIndexException {
        ...
        return (bfileChannelIn != null) && (bfileChannelOut == null);
    }
    ...
    public void writeBlock(byte[] bytes) throws ArUnvalidIndexException, ArFileException {
        ...
        if (bfileChannelOut == null) {
            bfileChannelOut = ((ArkFsLegacy) this.storage.getLegacy()).getFileChannelOut(realFile, position);
        }
        if (bfileChannelOut == null) {
            this.deleteInternal();
            throw new ArFileException("Internal error, Doc is not ready");
        ...
        long bufferSize = 0;
        try {
            bufferSize = bfileChannelOut.write(byteBuffer);
        ...
        if (bufferSize != bytes.length) {
            logger.error("Error during write");
            closeFile();
            throw new ArFileException("Internal error, file is not ready");
        }
        position += bufferSize;
