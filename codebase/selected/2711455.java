...
public class TimestampTest extends DatabaseTestCase {
    ...
    public void testTimestamps0001() throws Exception {
        ...
        stmt.close();
        PreparedStatement pstmt = con.prepareStatement("insert into #t0001 values (?, '1998-03-09 15:35:06.4',        " + "                         ?, '1998-03-09 15:35:00')");
    ...
    public void testEmptyResults0012() throws Exception {
        ...
        stmt.close();
        PreparedStatement pstmt = con.prepareStatement("select mytime, mytime2 from #t0012");
        ResultSet rs = pstmt.executeQuery();
        assertNotNull(rs);
    ...
    public void testForBrowse0014() throws Exception {
        ...
        PreparedStatement pstmt = con.prepareStatement("insert into #t0014 values (?)");
    ...
    public void testPrimaryKeyReal0024() throws Exception {
        ...
        PreparedStatement pstmt = con.prepareStatement("insert into #t0024 (pk, type, b, str) values(?, 'prepared', 0, ?)");
        for (int i = 0; i < d.length; i++) {
    ...
    public void testTimestampTimeZone() throws SQLException {
        Statement stmt = con.createStatement();
        stmt.executeUpdate("CREATE TABLE #testTimestampTimeZone (" + "ref INT NOT NULL, " + "tstamp DATETIME NOT NULL)");
        stmt.close();
        ...
        PreparedStatement pstmt = con.prepareStatement("INSERT INTO #testTimestampTimeZone (ref, tstamp) VALUES (?, ?)");
        pstmt.setInt(1, 0);
        pstmt.setTimestamp(2, tsStart, calNY);
        assertEquals(1, pstmt.executeUpdate());
        pstmt.close();
        pstmt = con.prepareStatement("SELECT * FROM #testTimestampTimeZone WHERE ref = ?");
        pstmt.setInt(1, 0);
        ResultSet rs = pstmt.executeQuery();
        assertTrue(rs.next());
    ...
    public void testTimestampRange() throws SQLException {
        Statement stmt = con.createStatement();
        stmt.executeUpdate("CREATE TABLE #testTimestampRange (id INT, d DATETIME)");
        PreparedStatement pstmt = con.prepareStatement("INSERT INTO #testTimestampRange VALUES (?, ?)");
        pstmt.setInt(1, 1);
        try {
            pstmt.setDate(2, Date.valueOf("0012-03-03"));
            pstmt.executeUpdate();
    ...
    public void testDayOfWeek() throws Exception {
        PreparedStatement pstmt = con.prepareStatement("SELECT {fn dayofweek({fn curdate()})}");
        ResultSet rs = pstmt.executeQuery();
        assertNotNull(rs);
        assertTrue(rs.next());
        int day = rs.getInt(1);
     ...
     */
    public long getTimeInMs(ResultSet rs) throws SQLException {
