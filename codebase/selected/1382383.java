...
public class WMFPainter extends AbstractWMFPainter {
    ...
    private void fillTextBackground(int align, int flag, Graphics2D g2d, float x, float y, float width, TextLayout layout) {
        float _x = x;
        if (align == WMFConstants.TA_CENTER) _x = x - width / 2; else if (align == WMFConstants.TA_RIGHT) _x = x - width;
    ...
    private void fillPolyPolygon(Graphics2D g2d, List pols) {
        if (pols.size() == 1) g2d.fill((Polygon2D) (pols.get(0))); else {
    ...
    private void setStroke(Graphics2D g2d, int penStyle, float penWidth, float scale) {
        float _width;
        if (penWidth == 0) _width = 1; else _width = penWidth;
        float _scale = (float) Toolkit.getDefaultToolkit().getScreenResolution() / currentStore.getMetaFileUnitsPerInch();
    ...
    private int getHorizontalAlignement(int align) {
        ...
        v = v % 24;
        v = v % 8;
        if (v >= 6) return WMFConstants.TA_CENTER; else if (v >= 2) return WMFConstants.TA_RIGHT; else return WMFConstants.TA_LEFT;
    }
    ...
    private void setBrushPaint(WMFRecordStore currentStore, Graphics2D g2d, int brushObject) {
        if (brushObject >= 0) {
            ...
            if (gdiObj.obj instanceof Color) g2d.setColor((Color) gdiObj.obj); else if (gdiObj.obj instanceof Paint) g2d.setPaint((Paint) gdiObj.obj); else g2d.setPaint(getPaint((byte[]) gdiObj.obj));
            brushObject = -1;
        }
    }
    ...
    private Paint getStoredPaint(WMFRecordStore currentStore, int object) {
        if (object >= 0) {
            ...
            if (gdiObj.obj instanceof Paint) return (Paint) gdiObj.obj; else return getPaint((byte[]) gdiObj.obj);
        } else return null;
    }
    ...
    private float getVerticalAlignmentValue(TextLayout layout, int vertAlign) {
        if (vertAlign == WMFConstants.TA_BOTTOM) return -layout.getDescent(); else if (vertAlign == WMFConstants.TA_TOP) return layout.getAscent(); else return 0;
    }
