...
public class StdSpaceShip implements Area, SpaceObject, SpaceShip {
    ...
    protected void cloneFix(StdSpaceShip E) {
        ...
        for (int b = 0; b < E.numBehaviors(); b++) {
            ...
            if (B != null) behaviors.addElement(B);
        }
        for (int a = 0; a < E.numEffects(); a++) {
            Ability A = E.fetchEffect(a);
            if (A != null) affects.addElement(A.copyOf());
        }
    ...
    public void affectEnvStats(Environmental affected, EnvStats affectableStats) {
        if (envStats().sensesMask() > 0) affectableStats.setSensesMask(affectableStats.sensesMask() | envStats().sensesMask());
    ...
    public void dockHere(Room R) {
        ...
        for (Enumeration e = getProperMap(); e.hasMoreElements(); ) {
            ...
            if (R2 != null) for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
                if ((R2.getRawExit(d) != null) && ((R2.rawDoors()[d] == null) || (R2.rawDoors()[d].getArea() != this)) && (R2.getRawExit(d).ID().endsWith("AirLock"))) {
                    airLockRoom = R2;
                    R2.rawDoors()[d] = null;
                    airLockDir = d;
                    break;
        ...
        if (airLockRoom != null) {
            if (airLockRoom.rawDoors()[airLockDir] == null) airLockRoom.rawDoors()[airLockDir] = R;
            if (airLockRoom.getRawExit(airLockDir) == null) airLockRoom.setRawExit(airLockDir, CMClass.getExit("GenAirLock"));
    ...
    public void unDock(boolean toSpace) {
        ...
        for (int i = 0; i < dock.numItems(); i++) {
            ...
            if (I.Name().equals(Name())) I.destroy();
        }
        for (Enumeration e = getProperMap(); e.hasMoreElements(); ) {
            Room R = (Room) e.nextElement();
            if (R != null) for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
                if (R.rawDoors()[d] == dock) R.rawDoors()[d] = null;
            }
        }
        dock = null;
        if (toSpace) {
    ...
    public String getNewRoomID(Room startRoom, int direction) {
        ...
        try {
            for (Enumeration r = CMLib.map().rooms(); r.hasMoreElements(); ) {
                ...
                if ((R.getArea().Name().equals(Name())) && (R.roomID().startsWith(Name() + "#"))) {
        ...
        if ((highest < 0) && (CMLib.map().getRoom(Name() + "#0")) == null) return Name() + "#0";
        if (lowest > highest) lowest = highest + 1;
        for (int i = lowest; i <= highest + 1000; i++) {
            if ((!allNums.containsKey(Integer.valueOf(i))) && (CMLib.map().getRoom(Name() + "#" + i) == null)) return Name() + "#" + i;
        }
    ...
    public StringBuffer getAreaStats() {
        return new StringBuffer("This is a space ship");
    }
            ...
            if (!myRooms.contains(R)) {
                ...
                for (int i = 0; i < myRooms.size(); i++) {
                    ...
                    if (R2.roomID().compareToIgnoreCase(R.roomID()) >= 0) {
                        if (R2.ID().compareToIgnoreCase(R.roomID()) == 0) myRooms.setElementAt(R, i); else myRooms.insertElementAt(R, i);
                        return;
                    }
                }
    ...
    public void delProperRoom(Room R) {
        ...
        synchronized (myRooms) {
            if (myRooms.removeElement(R)) delProperRoomnumber(R.roomID());
        }
    }

    public void addProperRoomnumber(String roomID) {
        if ((roomID != null) && (roomID.length() > 0)) getProperRoomnumbers().add(roomID);
    }

    public void delProperRoomnumber(String roomID) {
        if ((roomID != null) && (roomID.length() > 0)) getProperRoomnumbers().remove(roomID);
    }
    ...
    protected int getCodeNum(String code) {
        for (int i = 0; i < CODES.length; i++) if (code.equalsIgnoreCase(CODES[i])) return i;
        return -1;
    }
