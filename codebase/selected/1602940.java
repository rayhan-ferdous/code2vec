...
import java.util.*;
import com.creditors.vo.PF37VO;
import com.util.comparator.*;
import com.util.table.*;

public class PF37Model implements ITableModel {

    protected PF37VO fTotal;

    protected PF37VO[] fTableData;
    ...
    public Object getValueAt(int pRow, String pColumn) {
        PF37VO vo = fTableData[fSortOrder[pRow]];
        ...
        if ("taxCode".equals(pColumn)) return vo.getTaxCode();
        if ("flagUrgent".equals(pColumn)) return vo.getFlagUrgent();
    ...
    public Object getTotalValueAt(String pColumn) {
        if (fTotal != null) {
            ...
            if ("taxCode".equals(pColumn)) return fTotal.getTaxCode();
            if ("flagUrgent".equals(pColumn)) return fTotal.getFlagUrgent();
    ...
    public void sort(String pColumn, String pSortDirection) {
        ...
        if ("taxCode".equals(pColumn)) {
            fComparator = new StringComparator();
            String[] temp = new String[fTableData.length];
            for (int i = 0; i < temp.length; i++) {
                temp[i] = fTableData[i].getTaxCode();
            }
            sort(temp, 0, temp.length - 1, up);
        }
        if ("flagUrgent".equals(pColumn)) {
            fComparator = new StringComparator();
            String[] temp = new String[fTableData.length];
            for (int i = 0; i < temp.length; i++) {
                temp[i] = fTableData[i].getFlagUrgent();
            }
            sort(temp, 0, temp.length - 1, up);
        }
