...
class SpeedManagerPingMapperImpl implements SpeedManagerPingMapper {
    ...
    private static final int SPEED_HISTORY_PERIOD = 3 * 60 * 1000;
    ...
    protected synchronized limitEstimate getEstimatedLimit(boolean up) {
        ...
        limitEstimate result = new limitEstimate(estimate_speed, SpeedManagerLimitEstimate.TYPE_ESTIMATED, convertMetricToRating(estimate_var), estimate_hits, SystemTime.getCurrentTime(), (int[][]) segments.toArray(new int[segments.size()][]));
        return (result);
    }
    ...
    class region implements SpeedManagerPingZone {
    ...
    class limitEstimate implements SpeedManagerLimitEstimate, Cloneable {
        ...
        public String getString() {
            return ("speed=" + DisplayFormatters.formatByteCountToKiBEtc(speed) + ",metric=" + metric_rating + ",segs=" + segs.length + ",hits=" + hits + ",when=" + when);
        }
    }
