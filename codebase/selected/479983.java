...
public class DictionaryBackupAgent extends BackupHelperAgent {
    ...
    private static final String[] PROJECTION = { Words._ID, Words.WORD, Words.FREQUENCY, Words.LOCALE, Words.APP_ID };

    @Override
    public void onBackup(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState) throws IOException {
        byte[] userDictionaryData = getDictionary();
        long[] stateChecksums = readOldChecksums(oldState);
        stateChecksums[STATE_DICTIONARY] = writeIfChanged(stateChecksums[STATE_DICTIONARY], KEY_DICTIONARY, userDictionaryData, data);
    ...
    private byte[] getDictionary() {
        Cursor cursor = getContentResolver().query(Words.CONTENT_URI, PROJECTION, null, null, Words.WORD);
