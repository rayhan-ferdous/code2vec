...
public class DFDBKB extends DBKB {
        ...
        if (!tableExists(name)) {
            ...
            try {
                ...
                for (int i = 0; i < entries.length; i++) {
                    sql += entries[i];
                    if (i < entries.length - 1) sql += ", "; else sql += ")";
                }
    ...
    protected void createIndices() {
        ...
        try {
            ...
            if (logger.isLoggable(Logger.FINE)) logger.log(Logger.FINE, "Indices for DF tables couldn't be created", e);
        } finally {
            if (stmt != null) {
                try {
                    stmt.close();
    ...
    protected void createDFTables() {
        ...
        createTable(ONTOLOGY, new String[] { "descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "ontology VARCHAR(32)", "PRIMARY KEY( descrid, ontology )", "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )" });
        createTable(PROTOCOL, new String[] { "descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "protocol VARCHAR(32)", "PRIMARY KEY( descrid, protocol )", "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )" });
        ...
        createTable(SERVICEPROTOCOL, new String[] { "serviceid VARCHAR(" + MAX_PROP_LENGTH + ")", "protocol VARCHAR(32)", "PRIMARY KEY( serviceid, protocol )", "FOREIGN KEY( serviceid ) REFERENCES service( id )" });
        createTable(SERVICEONTOLOGY, new String[] { "serviceid VARCHAR(" + MAX_PROP_LENGTH + ")", "ontology VARCHAR(32)", "PRIMARY KEY( serviceid, ontology )", "FOREIGN KEY( serviceid ) REFERENCES service( id )" });
        ...
        createIndices();
        if (logger.isLoggable(Logger.FINE)) logger.log(Logger.FINE, "Tables correctly created");
    }
            ...
            while (iter.hasNext()) {
                ...
                while (iterS.hasNext()) {
                    ...
                    try {
                        ...
                        if (logger.isLoggable(Logger.SEVERE)) logger.log(Logger.SEVERE, "Cannot serialize property '" + prop.getName() + "' for service '" + service.getName() + "'", e);
                    }
                }
            }
    ...
    private void remove(String aid) throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (logger.isLoggable(Logger.FINE)) logger.log(Logger.FINE, "No DF description found to remove for agent '" + aid + "'");
            }
        } catch (SQLException sqle) {
            try {
                conn.rollback();
    ...
    private String createSelect(DFAgentDescription dfdTemplate) throws Exception {
        ...
        while (iter.hasNext()) {
            ...
            while (iterS.hasNext()) {
                ...
                Property prop = (Property) iterS.next();
                if (prop.getName() != null) lWhere.add(tmp1 + ".propkey='" + prop.getName() + "'");
                Object value = prop.getValue();
                if (value != null) {
                    if (needSerialization(value)) {
    ...
    private void cleanExpiredRegistrations() {
        ...
        try {
            ...
            if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "Error cleaning expired DF registrations", se);
        } finally {
            closeResultSet(rs);
        }
    }
    ...
    protected void unsubscribeSingle(SubscriptionResponder.Subscription sub) throws SQLException {
        ...
        if (!deleted) if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "No subscription to delete.");
    }
    ...
    private void closeStatement(Statement s) {
        try {
            ...
            if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "Closing SQL statement failed.");
        }
    }
