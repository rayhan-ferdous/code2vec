...
class NativeImageFormat {
    ...
    private static final int INT_RGB_LCMS_FMT = colorspaceSh(PT_RGB) | extraSh(1) | channelsSh(3) | bytesSh(1) | doswapSh(1) | swapfirstSh(1);
    ...
    private static final int FOUR_BYTE_ABGR_LCMS_FMT = colorspaceSh(PT_RGB) | extraSh(1) | channelsSh(3) | bytesSh(1) | doswapSh(1);
    ...
    public NativeImageFormat(Object imgData, int nChannels, int nRows, int nCols) {
        if (imgData instanceof short[]) {
            ...
            cmmFormat |= bytesSh(1);
        } else throw new IllegalArgumentException(Messages.getString("awt.47"));
    ...
    private static int getFormatFromSPPSampleModel(SinglePixelPackedSampleModel sm, boolean hasAlpha) {
        ...
        int pixelSize = 0;
        if (sm.getDataType() == DataBuffer.TYPE_USHORT) pixelSize = 2; else if (sm.getDataType() == DataBuffer.TYPE_INT) pixelSize = 4; else return 0;
        int bytes = 0;
        switch(mask) {
            case 0xFF:
                bytes = 1;
