...
public class StdArea implements Area {
    ...
    public void setCurrency(String newCurrency) {
        if (currency.length() > 0) {
            ...
            for (Enumeration e = CMLib.map().areas(); e.hasMoreElements(); ) CMLib.beanCounter().getCurrencySet(((Area) e.nextElement()).getCurrency());
        } else {
            currency = newCurrency;
    ...
    public String getBlurbFlag(String flag) {
        if ((flag == null) || (flag.trim().length() == 0)) return null;
        flag = flag.toUpperCase().trim() + " ";
        Vector V = allBlurbFlags();
        for (int i = 0; i < V.size(); i++) if (((String) V.elementAt(i)).startsWith(flag)) return ((String) V.elementAt(i)).substring(flag.length());
        return null;
    }
    ...
    public void addBlurbFlag(String flagPlusDesc) {
        ...
        if (getBlurbFlag(flag) == null) blurbFlags.addElement((flag + " " + flagPlusDesc).trim());
    }
    ...
    public void delBlurbFlag(String flagOnly) {
        ...
        flagOnly += " ";
        try {
            for (int v = 0; v < blurbFlags.size(); v++) if (((String) blurbFlags.elementAt(v)).startsWith(flagOnly)) {
    ...
    protected void finalize() {
        CMClass.unbumpCounter(this, CMClass.OBJECT_AREA);
    }
    ...
    public boolean savable() {
        return ((!amDestroyed) && (!CMath.bset(flags(), Area.FLAG_INSTANCE_CHILD)));
    }
    ...
    public synchronized RoomnumberSet getProperRoomnumbers() {
        if (properRoomIDSet == null) properRoomIDSet = (RoomnumberSet) CMClass.getCommon("DefaultRoomnumberSet");
    ...
    public RoomnumberSet getCachedRoomnumbers() {
        ...
        synchronized (properRooms) {
            ...
            for (int p = properRooms.size() - 1; p >= 0; p--) {
                ...
                if (R.roomID().length() > 0) set.add(R.roomID());
            }
        }
        return set;
    }
    ...
    public void setAreaState(int newState) {
        if ((newState == 0) && (!CMLib.threads().isTicking(this, Tickable.TICKID_AREA))) CMLib.threads().startTickDown(this, Tickable.TICKID_AREA, 1);
        flag = newState;
    }
    ...
    public boolean amISubOp(String username) {
        for (int s = subOps.size() - 1; s >= 0; s--) {
            if (((String) subOps.elementAt(s)).equalsIgnoreCase(username)) return true;
        }
        return false;
    }
    ...
    public void delSubOp(String username) {
        for (int s = subOps.size() - 1; s >= 0; s--) {
            if (((String) subOps.elementAt(s)).equalsIgnoreCase(username)) subOps.removeElementAt(s);
        }
    }
    ...
    public String getNewRoomID(Room startRoom, int direction) {
        ...
        if (highest < 0) for (int i = 0; i < Integer.MAX_VALUE; i++) {
            if ((CMLib.map().getRoom(Name() + "#" + i)) == null) return Name() + "#" + i;
        }
        if (lowest > highest) lowest = highest + 1;
        for (int i = lowest; i <= highest + 1000; i++) {
            if ((!set.contains(i)) && (CMLib.map().getRoom(Name() + "#" + i) == null)) return Name() + "#" + i;
        }
        return Name() + "#" + (int) Math.round(Math.random() * Integer.MAX_VALUE);
    }

    public CMObject newInstance() {
        if (CMSecurity.isDisabled("FATAREAS") && (ID().equals("StdArea"))) {
    ...
    protected void cloneFix(StdArea E) {
        ...
        parents = null;
        if (E.parents != null) parents = (Vector) E.parents.clone();
        children = null;
        if (E.children != null) children = (Vector) E.children.clone();
        if (E.blurbFlags != null) blurbFlags = (Vector) E.blurbFlags.clone();
        affects = new Vector(1);
        behaviors = new Vector(1);
        scripts = new Vector(1);
        for (int b = 0; b < E.numBehaviors(); b++) {
            Behavior B = E.fetchBehavior(b);
            if (B != null) behaviors.addElement((Behavior) B.copyOf());
        }
        for (int a = 0; a < E.numEffects(); a++) {
            Ability A = E.fetchEffect(a);
            if (A != null) affects.addElement((Ability) A.copyOf());
        }
    ...
    protected String finalPrejudiceFactors(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            String s = finalPrejudiceFactors((Area) e.nextElement());
            if (s.length() != 0) return s;
        }
        return "";
    }
    ...
    protected String[] finalItemPricingAdjustments(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            String[] s = finalItemPricingAdjustments((Area) e.nextElement());
            if (s.length != 0) return s;
        }
        return empty;
    }
    ...
    protected String finalIgnoreMask(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            String s = finalIgnoreMask((Area) e.nextElement());
            if (s.length() != 0) return s;
        }
        return "";
    }
    ...
    protected String finalBudget(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            String s = finalBudget((Area) e.nextElement());
            if (s.length() != 0) return s;
        }
        return "";
    }
    ...
    protected String finalDevalueRate(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            String s = finalDevalueRate((Area) e.nextElement());
            if (s.length() != 0) return s;
        }
        return "";
    }
    ...
    protected int finalInvResetRate(Area A) {
        ...
        for (Enumeration e = A.getParents(); e.hasMoreElements(); ) {
            int x = finalInvResetRate((Area) e.nextElement());
            if (x != 0) return x;
        }
        return 0;
    }
    ...
    public void setMiscText(String newMiscText) {
        miscText = "";
        if (newMiscText.trim().length() > 0) CMLib.coffeeMaker().setPropertiesStr(this, newMiscText, true);
    }
    ...
    public boolean okMessage(Environmental myHost, CMMsg msg) {
        ...
        for (int b = 0; b < numBehaviors(); b++) {
            N = fetchBehavior(b);
            if ((N != null) && (!N.okMessage(this, msg))) return false;
        }
        ...
        if (!msg.source().isMonster()) {
            ...
            if ((flag == Area.STATE_PASSIVE) && ((msg.sourceMinor() == CMMsg.TYP_ENTER) || (msg.sourceMinor() == CMMsg.TYP_LEAVE) || (msg.sourceMinor() == CMMsg.TYP_FLEE))) flag = Area.STATE_ACTIVE;
        }
        if ((flag >= Area.STATE_FROZEN) || (!CMLib.flags().allowsMovement(this))) {
            if ((msg.sourceMinor() == CMMsg.TYP_ENTER) || (msg.sourceMinor() == CMMsg.TYP_LEAVE) || (msg.sourceMinor() == CMMsg.TYP_FLEE)) return false;
        }
        if (parents != null) for (int i = 0; i < parents.size(); i++) if (!((Area) parents.elementAt(i)).okMessage(myHost, msg)) return false;
        if ((getTechLevel() > 0) && (!CMath.bset(getTechLevel(), Area.THEME_FANTASY))) {
            if ((CMath.bset(msg.sourceCode(), CMMsg.MASK_MAGIC)) || (CMath.bset(msg.targetCode(), CMMsg.MASK_MAGIC)) || (CMath.bset(msg.othersCode(), CMMsg.MASK_MAGIC))) {
                Room room = null;
                if ((msg.target() != null) && (msg.target() instanceof MOB) && (((MOB) msg.target()).location() != null)) room = ((MOB) msg.target()).location(); else if ((msg.source() != null) && (msg.source().location() != null)) room = msg.source().location();
                if (room != null) {
                    if (room.getArea() == this) room.showHappens(CMMsg.MSG_OK_VISUAL, "Magic doesn't seem to work here."); else room.showHappens(CMMsg.MSG_OK_VISUAL, "Magic doesn't seem to work there.");
                }
                return false;
            }
        } else if ((getTechLevel() > 0) && (!CMath.bset(getTechLevel(), Area.THEME_TECHNOLOGY))) {
            if ((msg.tool() != null) && (msg.tool() instanceof Electronics)) {
                    ...
                    default:
                        {
                            Room room = null;
                            if ((msg.target() != null) && (msg.target() instanceof MOB) && (((MOB) msg.target()).location() != null)) room = ((MOB) msg.target()).location(); else if ((msg.source() != null) && (msg.source().location() != null)) room = msg.source().location();
                            if (room != null) room.showHappens(CMMsg.MSG_OK_VISUAL, "Technology doesn't seem to work here.");
                            return false;
                        }
                }
            }
    ...
    public void executeMsg(Environmental myHost, CMMsg msg) {
        ...
        for (int b = 0; b < numBehaviors(); b++) {
            N = fetchBehavior(b);
            if (N != null) N.executeMsg(this, msg);
        }
        for (int s = 0; s < numScripts(); s++) {
            N = fetchScript(s);
            if (N != null) N.executeMsg(this, msg);
        ...
        if ((msg.sourceMinor() == CMMsg.TYP_RETIRE) && (amISubOp(msg.source().Name()))) delSubOp(msg.source().Name());
        if (parents != null) for (int i = 0; i < parents.size(); i++) ((Area) parents.elementAt(i)).executeMsg(myHost, msg);
    }
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if (tickID == Tickable.TICKID_AREA) {
            if ((flag <= Area.STATE_ACTIVE) && ((System.currentTimeMillis() - lastPlayerTime) > Area.TIME_PASSIVE_LAPSE)) {
                if (CMSecurity.isDisabled("PASSIVEAREAS")) lastPlayerTime = System.currentTimeMillis(); else flag = Area.STATE_PASSIVE;
            }
            ...
            for (int s = 0; s < numScripts(); s++) {
                ...
                if (S != null) S.tick(ticking, tickID);
            }
            int a = 0;
            while (a < numEffects()) {
                Ability A = fetchEffect(a);
                if (A != null) {
                    ...
                    int s = affects.size();
                    if (!A.tick(ticking, tickID)) A.unInvoke();
                    if (affects.size() == s) a++;
                } else a++;
            }
        }
    ...
    public void affectEnvStats(Environmental affected, EnvStats affectableStats) {
        ...
        int disposition = envStats().disposition() & ((Integer.MAX_VALUE - (EnvStats.IS_SLEEPING | EnvStats.IS_HIDDEN)));
        if ((affected instanceof Room) && (CMLib.map().hasASky((Room) affected))) {
            Climate C = getClimateObj();
            if ((C == null) || (C.weatherType((Room) affected) == Climate.WEATHER_BLIZZARD) || (C.weatherType((Room) affected) == Climate.WEATHER_DUSTSTORM) || (getTimeObj().getTODCode() == TimeClock.TIME_NIGHT)) disposition = disposition | EnvStats.IS_DARK;
        }
        if (disposition > 0) affectableStats.setDisposition(affectableStats.disposition() | disposition);
        affectableStats.setWeight(affectableStats.weight() + envStats().weight());
        for (int a = 0; a < numEffects(); a++) {
            Ability A = fetchEffect(a);
            if ((A != null) && (A.bubbleAffect())) A.affectEnvStats(affected, affectableStats);
        }
    }
    ...
    public void affectCharStats(MOB affectedMob, CharStats affectableStats) {
        for (int a = 0; a < numEffects(); a++) {
            Ability A = fetchEffect(a);
            if ((A != null) && (A.bubbleAffect())) A.affectCharStats(affectedMob, affectableStats);
        }
    }
    ...
    public void affectCharState(MOB affectedMob, CharState affectableMaxState) {
        for (int a = 0; a < numEffects(); a++) {
            Ability A = fetchEffect(a);
            if ((A != null) && (A.bubbleAffect())) A.affectCharState(affectedMob, affectableMaxState);
        }
    }
    ...
    public void delEffect(Ability to) {
        ...
        if (affects.size() < size) to.setAffectedOne(null);
    }
    ...
    public Ability fetchEffect(String ID) {
        for (int a = 0; a < numEffects(); a++) {
            Ability A = fetchEffect(a);
            if ((A != null) && (A.ID().equals(ID))) return A;
        }
        return null;
    }
    ...
    public boolean inMyMetroArea(Area A) {
        ...
        for (int i = 0; i < getNumChildren(); i++) if (getChild(i).inMyMetroArea(A)) return true;
        return false;
    }
    ...
    public void fillInAreaRooms() {
        for (Enumeration r = getProperMap(); r.hasMoreElements(); ) {
            ...
            if (R.roomID().length() > 0) {
                if (R instanceof GridLocale) ((GridLocale) R).buildGrid();
            }
        }
        for (Enumeration r = getProperMap(); r.hasMoreElements(); ) {
            Room R = (Room) r.nextElement();
    ...
    public void addScript(ScriptingEngine S) {
        ...
        if (!scripts.contains(S)) {
            ...
            for (int s = 0; s < scripts.size(); s++) {
                ...
                if ((S2 != null) && (S2.getScript().equalsIgnoreCase(S.getScript()))) return;
            }
    ...
    public void delScript(ScriptingEngine S) {
        if (scripts != null) {
            ...
            if (scripts.size() == 0) scripts = new Vector(1);
        }
    }
    ...
    public int[] getAreaIStats() {
        if (!CMProps.getBoolVar(CMProps.SYSTEMB_MUDSTARTED)) return new int[Area.AREASTAT_NUMBER];
        ...
        synchronized (("STATS_" + Name()).intern()) {
            ...
            for (Enumeration e = CMLib.factions().factions(); e.hasMoreElements(); ) {
                ...
                if (F.showInSpecialReported()) theFaction = F;
            }
            ...
            for (Enumeration r = getProperMap(); r.hasMoreElements(); ) {
                ...
                if (R instanceof GridLocale) statData[Area.AREASTAT_VISITABLEROOMS]--;
                for (int i = 0; i < R.numInhabitants(); i++) {
                    ...
                    if ((mob != null) && (mob.isMonster())) {
                        ...
                        if (!CMLib.flags().isAnimalIntelligence(mob)) statData[Area.AREASTAT_INTLEVEL] += lvl;
                        if (lvl < statData[Area.AREASTAT_MINLEVEL]) statData[Area.AREASTAT_MINLEVEL] = lvl;
                        if (lvl > statData[Area.AREASTAT_MAXLEVEL]) statData[Area.AREASTAT_MAXLEVEL] = lvl;
                    }
                }
            }
    ...
    public synchronized StringBuffer getAreaStats() {
        if (!CMProps.getBoolVar(CMProps.SYSTEMB_MUDSTARTED)) return new StringBuffer("");
        StringBuffer s = (StringBuffer) Resources.getResource("HELP_" + Name().toUpperCase());
        if (s != null) return s;
        s = new StringBuffer("");
        int[] statData = getAreaIStats();
        s.append(description() + "\n\r");
        if (author.length() > 0) s.append("Author         : " + author + "\n\r");
        ...
        if (statData[Area.AREASTAT_POPULATION] == 0) {
            if (getProperRoomnumbers().roomCountAllAreas() / 2 < properRooms.size()) s.append("Population     : 0\n\r");
        } else {
            s.append("Population     : " + statData[Area.AREASTAT_POPULATION] + "\n\r");
            String currName = CMLib.beanCounter().getCurrency(this);
            if (currName.length() > 0) s.append("Currency       : " + CMStrings.capitalizeAndLower(currName) + "\n\r"); else s.append("Currency       : Gold coins (default)\n\r");
            LegalBehavior B = CMLib.law().getLegalBehavior(this);
            if (B != null) {
                String ruler = B.rulingOrganization();
                if (ruler.length() > 0) {
                    Clan C = CMLib.clans().getClan(ruler);
                    if (C != null) s.append("Controlled by  : " + C.typeName() + " " + C.name() + "\n\r");
                }
            }
    ...
    public Behavior fetchBehavior(String ID) {
        for (int b = 0; b < numBehaviors(); b++) {
            Behavior B = fetchBehavior(b);
            if ((B != null) && (B.ID().equalsIgnoreCase(ID))) return B;
        }
        return null;
    }
    ...
    public void addProperRoom(Room R) {
        ...
        synchronized (properRooms) {
            ...
            if (properRooms.size() > 0) {
                ...
                if (comp == 0) return;
                addMetroRoom(R);
                if (comp > 0) properRooms.insertElementAt(R, insertAt); else if (insertAt == properRooms.size() - 1) properRooms.addElement(R); else properRooms.insertElementAt(R, insertAt + 1);
    ...
    public void addMetroRoom(Room R) {
        if (R != null) {
            if (R.roomID().length() == 0) {
                if ((R.getGridParent() != null) && (R.getGridParent().roomID().length() > 0)) addMetroRoomnumber(R.getGridParent().getGridChildCode(R));
            } else addMetroRoomnumber(R.roomID());
        }
    }
    ...
    public void delMetroRoom(Room R) {
        if (R != null) {
            if (R.roomID().length() == 0) {
                if ((R.getGridParent() != null) && (R.getGridParent().roomID().length() > 0)) delMetroRoomnumber(R.getGridParent().getGridChildCode(R));
            } else delMetroRoomnumber(R.roomID());
        }
    }
    ...
    public void addMetroRoomnumber(String roomID) {
        if (metroRoomIDSet == null) metroRoomIDSet = (RoomnumberSet) getProperRoomnumbers().copyOf();
        if ((roomID != null) && (roomID.length() > 0) && (!metroRoomIDSet.contains(roomID))) {
            metroRoomIDSet.add(roomID);
            if (!CMath.bset(flags(), Area.FLAG_INSTANCE_CHILD)) for (int p = getNumParents() - 1; p >= 0; p--) getParent(p).addMetroRoomnumber(roomID);
        }
    }
    ...
    public void delMetroRoomnumber(String roomID) {
        if ((metroRoomIDSet != null) && (roomID != null) && (roomID.length() > 0) && (metroRoomIDSet.contains(roomID))) {
            ...
            if (!CMath.bset(flags(), Area.FLAG_INSTANCE_CHILD)) for (int p = getNumParents() - 1; p >= 0; p--) getParent(p).delMetroRoomnumber(roomID);
        }
    }

    public boolean isRoom(Room R) {
        if (R == null) return false;
        if (R.roomID().length() > 0) return getProperRoomnumbers().contains(R.roomID());
    ...
    public void delProperRoom(Room R) {
        ...
        if (properRooms != null) synchronized (properRooms) {
            if (R.roomID().length() == 0) {
                if ((R.getGridParent() != null) && (R.getGridParent().roomID().length() > 0)) {
            ...
            } else if (properRooms.removeElement(R)) {
                delMetroRoom(R);
    ...
    public int metroSize() {
        int num = properSize();
        for (int c = getNumChildren() - 1; c >= 0; c--) num += getChild(c).metroSize();
        return num;
    }
    ...
    public int numberOfProperIDedRooms() {
        ...
        for (Enumeration e = getProperMap(); e.hasMoreElements(); ) {
            ...
            if (R.roomID().length() > 0) if (R instanceof GridLocale) num += ((GridLocale) R).xGridSize() * ((GridLocale) R).yGridSize(); else num++;
        }
        return num;
    }
    ...
    public Enumeration getProperMap() {
        ...
        for (int v = V.size() - 1; v >= 0; v--) {
            R = V.elementAt(v);
            if (R instanceof GridLocale) V.addAll(((GridLocale) R).getAllRooms());
        }
        return V.elements();
    }
    ...
    public Enumeration getFilledProperMap() {
        ...
        for (; r.hasMoreElements(); ) {
            R = (Room) r.nextElement();
            if (!V.contains(R)) V.addElement(R);
            for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
                R2 = R.rawDoors()[d];
                if ((R2 != null) && ((R2.roomID().length() == 0))) {
                    if (R2 instanceof GridLocale) V.addAll(((GridLocale) R2).getAllRooms()); else if (!V.contains(R2)) V.add(R2);
                }
            }
        }
        return V.elements();
    ...
    public Vector getMetroCollection() {
        ...
        for (int c = getNumChildren() - 1; c >= 0; c--) V.addAll(getChild(c).getMetroCollection());
        return V;
    }
    ...
    public Enumeration getMetroMap() {
        return getMetroCollection().elements();
    }
    ...
    public void addChildToLoad(String str) {
        childrenToLoad.addElement(str);
    }

    public void addParentToLoad(String str) {
        parentsToLoad.addElement(str);
    }
    ...
    public Area getChild(int num) {
        initChildren();
        return (Area) children.elementAt(num);
    }
    ...
    public Area getChild(String named) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if ((A.name().equalsIgnoreCase(named)) || (A.Name().equalsIgnoreCase(named))) return A;
        }
        return null;
    }
    ...
    public boolean isChild(String named) {
        ...
        for (int i = 0; i < children.size(); i++) {
            Area A = (Area) children.elementAt(i);
            if ((A.name().equalsIgnoreCase(named)) || (A.Name().equalsIgnoreCase(named))) return true;
        }
        return false;
    }
    ...
    public void removeChild(Area Disowned) {
        initChildren();
        children.removeElement(Disowned);
    }

    public void removeChild(int Disowned) {
        initChildren();
        children.removeElementAt(Disowned);
    }
    ...
    public Area getParent(int num) {
        initParents();
        return (Area) parents.elementAt(num);
    }
    ...
    public void removeParent(Area Disowned) {
        initParents();
        parents.removeElement(Disowned);
    }

    public void removeParent(int Disowned) {
        initParents();
        parents.removeElementAt(Disowned);
    }
    ...
    private static final String[] CODES = { "CLASS", "CLIMATE", "DESCRIPTION", "TEXT", "TECHLEVEL", "BLURBS", "PREJUDICE", "BUDGET", "DEVALRATE", "INVRESETRATE", "IGNOREMASK", "PRICEMASKS" };

    private static String[] codes = null;

    public String[] getStatCodes() {
        if (codes == null) codes = CMProps.getStatCodesList(CODES, this);
        return codes;
    }
    ...
    public String getStat(String code) {
        switch(getCodeNum(code)) {
            ...
            case 4:
                return "" + getTechLevel();
            case 5:
                return "" + CMLib.xml().getXMLList(blurbFlags);
            case 6:
                return prejudiceFactors();
            case 7:
                return budget();
    ...
    public void setStat(String code, String val) {
        switch(getCodeNum(code)) {
            case 0:
                return;
            case 1:
                setClimateType(CMath.s_parseBitIntExpression(Area.CLIMATE_DESCS, val));
                break;
            case 2:
                setDescription(val);
                break;
            case 3:
                ...
                break;
            case 4:
                setTechLevel(CMath.s_parseBitIntExpression(Area.THEME_DESCS, val));
                break;
            case 5:
                {
                    if (val.startsWith("+")) addBlurbFlag(val.substring(1)); else if (val.startsWith("-")) delBlurbFlag(val.substring(1)); else blurbFlags = CMLib.xml().parseXMLList(val);
                    break;
                }
            case 6:
    ...
    public boolean sameAs(Environmental E) {
        ...
        for (int i = 0; i < codes.length; i++) if (!E.getStat(codes[i]).equals(getStat(codes[i]))) return false;
        return true;
    }
}
