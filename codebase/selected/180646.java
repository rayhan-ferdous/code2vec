...
import java.util.*;
import com.be.vo.VATCodeVO;
import com.util.comparator.*;
import com.util.table.*;

public class VATCodeModel implements ITableModel {

    protected VATCodeVO fTotal;

    protected VATCodeVO[] fTableData;
    ...
    public Object getValueAt(int pRow, String pColumn) {
        VATCodeVO vo = fTableData[fSortOrder[pRow]];
    ...
    public Object getTotalValueAt(String pColumn) {
        if (fTotal != null) {
            ...
            if ("debit".equals(pColumn)) return new Long(fTotal.getDebit());
            if ("credit".equals(pColumn)) return new Long(fTotal.getCredit());
    ...
    public void sort(String pColumn, String pSortDirection) {
        ...
        if ("debit".equals(pColumn)) {
            Sorter sorter = new Sorter();
            long[] temp = new long[fTableData.length];
            for (int i = 0; i < temp.length; i++) {
                temp[i] = fTableData[i].getDebit();
            }
            fSortOrder = sorter.sortLong(temp, fSortOrder, up);
        }
        if ("credit".equals(pColumn)) {
            Sorter sorter = new Sorter();
            long[] temp = new long[fTableData.length];
            for (int i = 0; i < temp.length; i++) {
                temp[i] = fTableData[i].getCredit();
            }
            fSortOrder = sorter.sortLong(temp, fSortOrder, up);
        }
