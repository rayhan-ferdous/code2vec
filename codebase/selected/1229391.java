...
public class RunIceTagger {
    ...
    private int lineFormat = Segmentizer.tokenPerLine;
    ...
    protected IceLog logger = null;

    protected int numUnknowns = 0;
    ...
    protected String externalAnalysis = "icenlp";
    ...
    private void checkParameters() {
        ...
        if (lineFormatStr == null) {
            ...
            if (!lineFormatStr.matches("1|2|3")) {
                ...
                error = true;
            } else changedDefaultInputFormat = true;
        }
        if (outputFormatStr == null) {
            ...
            if (!outputFormatStr.matches("1|2")) {
                ...
                error = true;
            } else changedDefaultOutputFormat = true;
        }
    ...
    private void getFormat() {
        ...
        if (separatorStr.equals("space")) separator = " "; else separator = "_";
        if (modelTypeStr != null) {
            if (modelTypeStr.equals("start")) modelType = IceTagger.HmmModelType.start; else if (modelTypeStr.equals("end")) modelType = IceTagger.HmmModelType.end; else if (modelTypeStr.equals("startend")) modelType = IceTagger.HmmModelType.startend;
        }
    }
    ...
    protected void getParameters(String args[]) {
        for (int i = 0; i <= args.length - 1; i++) {
            if (args[i].equals("-help")) {
                printHeader();
                showParametersExit();
            } else if (args[i].equals("-i")) inputFile = args[i + 1]; else if (args[i].equals("-o")) outputFile = args[i + 1]; else if (args[i].equals("-f")) fileList = args[i + 1]; else if (args[i].equals("-lf")) {
            ...
            } else if (args[i].equals("-sep")) separatorStr = args[i + 1]; else if (args[i].equals("-ss")) sentenceStartStr = args[i + 1]; else if (args[i].equals("-l")) logFile = args[i + 1]; else if (args[i].equals("-bd")) baseDictPath = args[i + 1]; else if (args[i].equals("-d")) dictPath = args[i + 1]; else if (args[i].equals("-p")) idiomsDictPath = args[i + 1]; else if (args[i].equals("-vp")) verbPrepDictPath = args[i + 1]; else if (args[i].equals("-vo")) verbObjDictPath = args[i + 1]; else if (args[i].equals("-va")) verbAdverbDictPath = args[i + 1]; else if (args[i].equals("-be")) endingsBasePath = args[i + 1]; else if (args[i].equals("-e")) endingsDictPath = args[i + 1]; else if (args[i].equals("-ep")) endingsProperDictPath = args[i + 1]; else if (args[i].equals("-pr")) prefixesDictPath = args[i + 1]; else if (args[i].equals("-tf")) tagFrequencyFile = args[i + 1]; else if (args[i].equals("-tm")) tagMapFile = args[i + 1]; else if (args[i].equals("-td")) tokenDictPath = args[i + 1]; else if (args[i].equals("-lem")) lemmatize = true; else if (args[i].equals("-bt")) baseTagging = true; else if (args[i].equals("-nf")) fullDisambiguation = false; else if (args[i].equals("-fo")) fullOutput = true; else if (args[i].equals("-ns")) strictTokenization = false; else if (args[i].equals("-num")) sameTagForAllNumbers = false; else if (args[i].equals("-ner")) namedEntityRecognition = true; else if (args[i].equals("-mt")) modelTypeStr = args[i + 1]; else if (args[i].equals("-m")) modelStr = args[i + 1]; else if (args[i].equals("-x")) externalAnalysis = args[i + 1];
        }
        getFormat();
    }
    ...
    protected void printResults(BufferedWriter outFile) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            numTokens++;
            if (tok.isUnknown()) numUnknowns++;
            String str = iceOutput.buildOutput(tok, i, size);
            outFile.write(str);
            if (outputFormat == Segmentizer.tokenPerLine) outFile.newLine();
        }
        outFile.newLine();
    }

    private void printResultsBaseTagging(BufferedWriter outFile) throws IOException {
        ArrayList tokens = tokenizer.tokens;
    ...
    protected void tagText(BufferedWriter outFile) throws IOException {
        String sentence;
        int count = 0;
        String strCount = Integer.toString(count);
        if (!standardInputOutput) System.out.print("Tagging sentence nr 1: " + "\r");
        while (segmentizer.hasMoreSentences()) {
            ...
            if (!sentence.equals("")) {
                ...
                logger.log(output);
                if (lineFormat == Segmentizer.tokenPerLine) tokenizer.tokenizeSplit(sentence); else tokenizer.tokenize(sentence);
                if (tokenizer.tokens.size() > 0) {
                    tokenizer.splitAbbreviations();
                    tagger.tagTokens(tokenizer.tokens);
                    if (baseTagging) printResultsBaseTagging(outFile); else printResults(outFile);
                }
            }
        }
        outFile.flush();
    ...
    private void processParam(String args[]) throws IOException {
        ...
        if (args.length >= 1 && args[0].equals("-p")) {
            ...
            if (inputFile == null && fileList == null) {
                ...
                if (!changedDefaultInputFormat) lineFormat = Segmentizer.sentencePerLine;
                if (!changedDefaultOutputFormat) outputFormat = Segmentizer.sentencePerLine;
            }
        }
    }
    ...
    private void printInfoBeforeTagging() {
        ...
        if (dictPath != null && !dictPath.equals("")) System.out.println("Main lexicon: " + dictPath);
        if (fullDisambiguation) System.out.println("Performing full disambiguation"); else System.out.println("Not using full disambiguation");
        if (!(modelType == IceTagger.HmmModelType.none) && fullDisambiguation) {
            if (modelType == IceTagger.HmmModelType.startend) System.out.println("Using a HMM for initial and full disambiguation"); else if (modelType == IceTagger.HmmModelType.start) System.out.println("Using a HMM for initial disambiguation"); else if (modelType == IceTagger.HmmModelType.end) System.out.println("Using a HMM for full disambiguation");
        }
        if (lemmatize) System.out.println("Producing lemmata");
        if (tagMapFile != null) System.out.println("Mapping tags using " + tagMapFile);
        if (sentenceStart == IceTagger.sentenceStartUpperCase) System.out.println("Sentences start with an upper case letter"); else System.out.println("Sentences start with a lower case letter");
        if (fileList != null) System.out.println("Tagging files from filelist " + fileList);
    }

    private void getIceTaggerLexicons() throws IOException {
        InputStream isDictionaryBase, isDictionary, isEndingsBase, isEndings, isEndingsProper, isVerbPrep, isVerbObj, isVerbAdverb, isIdioms, isPrefixes, isTagFrequency;
        ...
        iceLex = new IceTaggerLexicons(isDictionaryBase, isDictionary, isEndingsBase, isEndings, isEndingsProper, isVerbPrep, isVerbObj, isVerbAdverb, isIdioms, isPrefixes, isTagFrequency);
    }

    private void getTriTaggerLexicons() throws IOException {
        if (modelStr != null && !modelStr.equals("")) triLex = new TriTaggerLexicons(modelStr, true, true); else {
    ...
    private void createAllObjects(int sentenceStart) throws IOException {
        ...
        if (externalAnalysis.equals("icenlp")) {
            if (inputFile != null) segmentizer = new Segmentizer(inputFile, lineFormat, tokLex); else if (standardInputOutput) {
        ...
        if (fullDisambiguation && !(modelType == IceTagger.HmmModelType.none)) {
            if (!standardInputOutput) {
                if (modelStr != null && !modelStr.equals("")) System.out.println("Loading HMM: " + modelStr); else System.out.println("Loading default HMM ...");
            }
        ...
        IceMorphy morphoAnalyzer = new IceMorphy(iceLex.morphyLexicons.dict, iceLex.morphyLexicons.baseDict, iceLex.morphyLexicons.endingsBase, iceLex.morphyLexicons.endings, iceLex.morphyLexicons.endingsProper, iceLex.morphyLexicons.prefixes, iceLex.morphyLexicons.tagFrequency, logger);
        tagger = new IceTagger(sentenceStart, logger, morphoAnalyzer, iceLex.morphyLexicons.baseDict, iceLex.morphyLexicons.dict, iceLex.idioms, iceLex.verbPrep, iceLex.verbObj, iceLex.verbAdverb, baseTagging, fullDisambiguation, triTagger, modelType);
    ...
    protected Date initialize(String args[]) throws IOException {
        processParam(args);
        if (sentenceStartStr.equals("upper")) sentenceStart = IceTagger.sentenceStartUpperCase; else sentenceStart = IceTagger.sentenceStartLowerCase;
        if (!standardInputOutput) {
            printHeader();
    ...
    protected void finish(Date before) {
        ...
        if (fileList != null) System.out.println();
        if (!standardInputOutput) {
