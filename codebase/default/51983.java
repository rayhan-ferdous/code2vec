...
public class RunExamples {
    ...
    public static void main(String... args) throws Exception {
        ...
        if (jtreg) {
            ...
            tmpDir = new File(System.getProperty("java.io.tmpdir"), RunExamples.class.getName() + (new SimpleDateFormat("yyMMddHHmmss")).format(new Date()));
        }
        Example.setTempDir(tmpDir);
        RunExamples r = new RunExamples();
        try {
            if (r.run(args)) return;
        } finally {
            if (tmpDir.isDirectory() && tmpDir.getName().startsWith(RunExamples.class.getName())) {
                if (clean(tmpDir)) tmpDir.delete();
            }
        }
        if (jtreg) throw new Exception(r.errors + " errors occurred"); else System.exit(1);
    }
    ...
    boolean run(String... args) {
        ...
        for (int i = 0; i < args.length; i++) {
            String arg = args[i];
            if (arg.equals("-k") && (i + 1) < args.length) selectedKeys.add(args[++i]); else if (arg.equals("-examples") && (i + 1) < args.length) examplesDir = new File(args[++i]); else if (arg.equals("-raw")) raw = true; else if (arg.equals("-showFiles")) showFiles = true; else if (arg.equals("-verbose")) verbose = true; else if (arg.equals("-o") && (i + 1) < args.length) outFile = new File(args[++i]); else if (arg.equals("-title") && (i + 1) < args.length) title = args[++i]; else if (arg.equals("-argtypes")) argTypes = true; else if (arg.startsWith("-")) {
        ...
        if (argTypes) Example.Compiler.factory = new ArgTypeCompilerFactory();
        if (selectedKeys.size() > 0) {
            Set<Example> examples = getExamples(examplesDir);
            nextKey: for (String k : selectedKeys) {
                for (Example e : examples) {
                    if (e.getDeclaredKeys().contains(k)) continue nextKey;
                }
                error("Key " + k + ": no examples found");
            }
        } else {
            if (selectedExamples.isEmpty()) selectedExamples = getExamples(examplesDir);
        }
        try {
            Runner r;
            if (outFile == null) {
            ...
            } else if (outFile.getName().endsWith(".html")) r = new HTMLRunner(outFile, showFiles, raw, verbose, title); else r = new TextRunner(outFile, showFiles, raw, verbose);
    ...
    Set<Example> getExamples(File examplesDir) {
        ...
        for (File f : examplesDir.listFiles()) {
            if (isValidExample(f)) results.add(new Example(f));
        }
        return results;
    }

    boolean isValidExample(File f) {
        return (f.isDirectory() && (!jtreg || f.list().length > 0)) || (f.isFile() && f.getName().endsWith(".java"));
    }
    ...
    abstract static class Runner {
        ...
        protected Pattern copyrightHeaderPat = Pattern.compile("(?s)(/\\*.*?Copyright.*?\\*/\n)\\s*(.*)");

        protected Pattern infoHeaderPat = Pattern.compile("(?s)((?://\\s*[a-z]+:[^\n]*\n)+)\\s*(.*)");
    ...
    static class TextRunner extends Runner {
        ...
        TextRunner(File file, boolean showFiles, boolean raw, boolean verbose) throws IOException {
            super(showFiles, raw, verbose);
            this.file = file;
            out = new PrintWriter(new FileWriter(file));
        }

        TextRunner(PrintWriter out, boolean showFiles, boolean raw, boolean verbose) throws IOException {
            super(showFiles, raw, verbose);
            this.out = out;
        }
        ...
        @Override
        void run(Example e) {
            if (showFiles) out.println("--- Output:");
    ...
    static class HTMLRunner extends Runner {
        ...
        HTMLRunner(File file, boolean showFiles, boolean raw, boolean verbose, String title) throws IOException {
            super(showFiles, raw, verbose);
            this.file = file;
        ...
        @Override
        void run(Collection<Example> examples) throws IOException {
            if (examples.size() > 1) writeIndex(examples);
            super.run(examples);
        }
