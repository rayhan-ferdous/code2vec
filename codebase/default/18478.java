...
public class GenStubs {
    ...
    boolean run(String... args) {
        ...
        for (ListIterator<String> iter = Arrays.asList(args).listIterator(); iter.hasNext(); ) {
            ...
            if (arg.equals("-s") && iter.hasNext()) outdir = new File(iter.next()); else if (arg.equals("-sourcepath") && iter.hasNext()) sourcepath = iter.next(); else if (arg.startsWith("-")) throw new IllegalArgumentException(arg); else {
                classes.add(arg);
    ...
    boolean run(String sourcepath, File outdir, List<String> classes) {
        if (sourcepath == null) throw new IllegalArgumentException("sourcepath not set");
        if (outdir == null) throw new IllegalArgumentException("source output dir not set");
        ...
        try {
            ...
            for (String c : classes) {
                ...
                if (fo == null) error("class not found: " + c); else files.add(fo);
            }
    ...
    List<File> splitPath(String path) {
        ...
        for (String p : path.split(File.pathSeparator)) {
            if (p.length() > 0) list.add(new File(p));
        }
        return list;
    }
    ...
    public static class Ant extends MatchingTask {
        ...
        public void setClasspath(Path cp) {
            if (classpath == null) classpath = cp; else classpath.append(cp);
        }
        ...
        String[] filter(File srcDir, File destDir, String[] files) {
            ...
            for (String f : files) {
                ...
                if (srcTime > destTime) results.add(f);
            }
