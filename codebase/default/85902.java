...
public class jnamed {
    ...
    public void addPrimaryZone(String zonefile) throws IOException {
    ...
    public RRset findExactMatch(Name name, short type, short dclass, boolean glue) {
        ...
        if (zone != null) return zone.findExactMatch(name, type); else {
            RRset[] rrsets;
            if (glue) rrsets = cache.findAnyRecords(name, type, dclass); else rrsets = cache.findRecords(name, type, dclass);
    ...
    private void addGlue(Message response, Name name) {
        ...
        while (e.hasMoreElements()) {
            Record r = (Record) e.nextElement();
            if (response.findRecord(r) == false) response.addRecord(r, Section.ADDITIONAL);
        }
        e = a.sigs();
        while (e.hasMoreElements()) {
            Record r = (Record) e.nextElement();
