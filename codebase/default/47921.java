...
public class jnamed {
    ...
    public RRset findExactMatch(Name name, short type, short dclass, boolean glue) {
        ...
        if (zone != null) return zone.findRecords(name, type); else if (glue) return cache.findAnyRecords(name, type, dclass); else return cache.findRecords(name, type, dclass);
    }

    void addRRset(Message response, RRset rrset) {
    ...
    void addAuthority(Message response, Name name, Zone zone) {
        if (response.getHeader().getCount(Section.ANSWER) > 0 || zone == null) {
            ...
            if (nsRecords == null) {
                if (zone != null) nsRecords = zone.getNS(); else nsRecords = cache.findRecords(Name.root, Type.NS, DClass.IN);
            }
    ...
    private void addGlue(Message response, Name name) {
        ...
        while (e.hasMoreElements()) {
            Record r = (Record) e.nextElement();
            if (response.findRecord(r) == false) response.addRecord(Section.ADDITIONAL, r);
        }
    }
    ...
    TSIG findTSIG(Name name) {
        ...
        if (key != null) return new TSIG(name.toString(), key); else return null;
    }
    ...
    public Message formerrMessage(byte[] in) {
        ...
        try {
            ...
            for (int i = 0; i < 4; i++) header.setCount(i, 0);
        } catch (IOException e) {
            header = new Header(0);
        }
        Message response = new Message();
