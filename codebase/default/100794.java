...
public class jnamed {
    ...
    public RRset findExactMatch(Name name, short type, short dclass, boolean glue) {
        ...
        if (zone != null) return zone.findRecords(name, type); else if (glue) return cache.findAnyRecords(name, type); else return cache.findRecords(name, type);
    }
    ...
    void addAuthority(Message response, Name name, Zone zone) {
        if (response.getHeader().getCount(Section.ANSWER) > 0 || zone == null) {
            ...
            if (nsRecords == null) {
                if (zone != null) nsRecords = zone.getNS(); else nsRecords = cache.findRecords(Name.root, Type.NS);
            }
