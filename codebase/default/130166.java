...
public class MessageInfo {
    ...
    public static void main(String... args) throws Exception {
        ...
        if (jtreg) throw new Exception(mi.errors + " errors occurred"); else System.exit(1);
    }
    ...
    boolean run(String... args) {
        ...
        for (int i = 0; i < args.length; i++) {
            String arg = args[i];
            if (arg.equals("-examples") && (i + 1) < args.length) examplesDir = new File(args[++i]); else if (arg.equals("-notyet") && (i + 1) < args.length) notYetFile = new File(args[++i]); else if (arg.equals("-ensureNewlines")) ensureNewlines = true; else if (arg.equals("-fixIndent")) fixIndent = true; else if (arg.equals("-sort")) sort = true; else if (arg.equals("-verbose")) verbose = true; else if (arg.equals("-replace")) replace = true; else if (arg.equals("-check")) check = true; else if (arg.equals("-o") && (i + 1) < args.length) outFile = new File(args[++i]); else if (arg.startsWith("-")) {
        ...
        for (Map.Entry<String, Set<String>> e : msgInfo.entrySet()) {
            ...
            if (!info.isEmpty()) {
                ...
                if (!replace) {
                    if (info.fields.size() != suggestedInfo.fields.size()) error("Cannot merge info for " + k); else suggestedInfo.merge(info);
                }
            }
            if (outFile == null) {
    ...
    void check(MessageFile mf, File notYetFile) {
        ...
        for (Map.Entry<String, MessageFile.Message> e : mf.messages.entrySet()) {
            ...
            if (m.needInfo() && m.getInfo().isEmpty()) {
                ...
                if (notYetList.contains(key)) System.err.println("Warning: no info for " + key); else error("no info for " + key);
            }
        }
    }
    ...
    void fixIndent(MessageFile mf) {
        for (MessageFile.Message m : mf.messages.values()) {
            ...
            while (l.text.endsWith("\\") && l.next != null) {
                if (!l.next.text.matches("^    \\S.*")) l.next.text = "    " + l.next.text.trim();
                l = l.next;
            }
        }
    }
    ...
    Set<String> getNotYetList(File file) {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                if (hash != -1) line = line.substring(0, hash).trim();
                if (line.matches("[A-Za-z0-9-_.]+")) results.add(line);
            }
        } catch (IOException e) {
            throw new Error(e);
        }
